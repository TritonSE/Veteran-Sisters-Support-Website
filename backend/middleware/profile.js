...
export const authenticateProfilePermissions = async (req, res, next) => {
    try{
        ...
        switch(user.role){
            case "veteran":
                if(!(user.email === profileUser.email)){
                    return res.status(500).json({ error: "You do not have permission to view this user!" });
                }   
                break;
            case "volunteer":
                ...
                break;
            case "staff":
                if(!user.assignedPrograms.some(element => profileUser.assignedPrograms.includes(element))){
                    return res.status(500).json({ error: "You do not have permission to view this user!" });
                }
                break;
            case "admin":
